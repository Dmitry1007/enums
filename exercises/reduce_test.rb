...
class ReduceTest < Minitest::Test
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    keywords = ["fish", "blue"]
    result = keywords.reduce('one fish two fish red fish blue fish') do |stringy,word|
      stringy.gsub!(word, word.upcase)
    end
  ...
  def test_subtract_smallest_values_from_100
    ...
    result = elements.reduce(100) { |sum,num| sum - num.min }
    assert_equal 88, result
  end

  def test_add_all_the_second_values_together
    elements = [["a", 1], ["b", 9], ["c", 21]]
    result = elements.reduce(0) { |sum, arry| sum + arry[-1] }
